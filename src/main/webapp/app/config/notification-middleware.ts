  ...
  return next(action)
    .then(response => {
      if (response && response.value && response.value.data) {
        ...
        if (res.shouldShowMessage && res.content) {
          if (res.status) {
    ...
    .catch(error => {
      ...
      if (action.meta && action.meta.responseMessage) {
        toast.error(action.meta.responseMessage);
            ...
            case 400:
              ...
              if (errorHeader) {
                ...
                for (i = 0; i < fieldErrors.length; i++) {
                  ...
                  const fieldName = translate(`appgwApp.${fieldError.objectName}.${convertedField}`);
